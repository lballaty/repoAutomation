...
# GitHub Personal Access Tokens for multiple accounts

GITHUB_PATS["repoaccount1"]="${GITHUB_PAT_1}"
GITHUB_PATS["repoaccount2"]="${GITHUB_PAT_2}"


# GitHub usernames linked to each PAT
declare -A GITHUB_USERS
GITHUB_USERS["repoaccount1"]="lballaty"
GITHUB_USERS["repoaccount2"]="searchingfool"

# Default GitHub account (choose one)
DEFAULT_GITHUB_ACCOUNT="repoaccount1"

# Validate the selected account exists
if [[ -z "${GITHUB_PATS[$DEFAULT_GITHUB_ACCOUNT]}" ]]; then
    echo "❌ ERROR: No valid GitHub PAT found for '$DEFAULT_GITHUB_ACCOUNT'. Please check your config."
    exit 1
fi

# Set GitHub credentials
GITHUB_PAT="${GITHUB_PATS[$DEFAULT_GITHUB_ACCOUNT]}"
GITHUB_USER="${GITHUB_USERS[$DEFAULT_GITHUB_ACCOUNT]}"
...
check_log_size() {
    ...
    if [[ -f "$LOG_FILE" ]]; then
        ...
        if (( LOG_SIZE > MAX_LOG_SIZE )); then
            warning "Log file exceeded $MAX_LOG_SIZE bytes. Consider rotating logs."
        fi
    else
...
sync_local_with_github() {
    ...
    for REPO in "$LOCAL_REPOS_DIR"/*/; do
        ...
        if [[ ! -d "$REPO/.git" ]]; then
            # Not a Git repo; record its name and skip
            echo "$(basename "$REPO")" >> "$NON_GIT_REPOS_FILE"
            continue
        fi
...
handle_orphaned_production_repos() {
    ...
    for REPO in "$PRODUCTION_DIR"/*/; do
        ...
        # If this folder is literally named "Archived", skip it
        if [[ "$REPO_NAME" == "Archived" ]]; then
            # Don't try to archive the archive folder itself
            continue
        fi

        # Check if it's orphaned:
        #  1) Not found locally, AND
        #  2) Not found in the list of GitHub repos
        if [[ ! -d "$LOCAL_REPOS_DIR/$REPO_NAME" && \
              ! $(curl -s -H "Authorization: token $GITHUB_PAT" \
                    "https://api.github.com/user/repos?per_page=100" \
                  | jq -r '.[].name' | grep -w "$REPO_NAME") ]]; then

            warning "Orphaned repository detected: $REPO_NAME"
            read -p "Do you want to archive it? (y/n): " CONFIRM

            if [[ "$CONFIRM" == "y" ]]; then
                # Move it inside ARCHIVE_DIR, e.g., Production/Archived/SomeRepo
...
check_and_clone_github_repos() {
    ...
    GITHUB_REPOS=$(curl -s -H "Authorization: token $GITHUB_PAT" "https://api.github.com/user/repos?per_page=100&page=1" | jq -r '.[].name' | tr '[:upper:]' '[:lower:]' | sort)
...
delete_repositories() {
    ...
    while IFS= read -r REPO_NAME || [[ -n "$REPO_NAME" ]]; do
        ...
        if [[ "$CONFIRM" == "y" ]]; then
            ...
            curl -X DELETE -H "Authorization: token $GITHUB_PAT" "https://api.github.com/repos/$GITHUB_USER/$REPO_NAME"
